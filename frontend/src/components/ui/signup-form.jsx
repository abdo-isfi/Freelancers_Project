...
export function SignupForm({ onSubmit, register, errors, loading }) {
  ...
  return (
    <div className="form-container sign-up-container h-full z-10 w-full px-4 lg:px-16 flex flex-col justify-center">
      <form className='text-center py-10 md:py-20 grid gap-2 h-full' onSubmit={onSubmit}>
        <div className='grid gap-4 md:gap-6 mb-2'>
          <h1 className='text-3xl md:text-4xl font-extrabold text-[var(--color-heading)]' onClick={(e) => {e.preventDefault()}}>Sign up</h1>
        </div>
      ...
      <div className='grid gap-4 items-center'>
          <div className="flex flex-col md:flex-row gap-4">
             <div className="w-full">
                ...
                />
                {errors?.firstName && (
                  <p className="text-red-500 text-xs text-left mt-1">{errors.firstName.message}</p>
                )}
             </div>
             <div className="w-full">
                <AppInput 
                ...
                />
                {errors?.lastName && (
                  <p className="text-red-500 text-xs text-left mt-1">{errors.lastName.message}</p>
                )}
             </div>
          </div>
          ...
          <div className="w-full">
            ...
            {errors?.confirmPassword && (
              <p className="text-red-500 text-xs text-left mt-1">{errors.confirmPassword.message}</p>
            )}
          </div>
        </div>
        ...
        <div className='flex gap-4 justify-center items-center mt-4'>
          ...
          >
          <span className="text-sm px-2 py-1">{loading ? 'Signing up...' : 'Sign Up'}</span>
